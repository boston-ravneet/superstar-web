...
import { getAccountById, mapAccountRecord, deleteAccountAndData } from "@/lib/db/accounts";
...
export async function DELETE(request: Request) {
  try {
    ...
    await deleteAccountAndData(bindings.DB, session.accountId);

    return jsonOk({ deleted: true });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Unable to delete account.";
    return jsonError(message, "ACCOUNT_DELETE_FAILED", 500);
  }
}
